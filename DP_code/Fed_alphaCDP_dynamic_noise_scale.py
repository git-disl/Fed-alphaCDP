...
for round in range(rounds):
    ...
    for k_t in range(parti_client_num):
        ...
        # ## next 5 lines for dynamic clipping
        # l2_norm_clip = np.max(np.asarray(norms))
        # l2_norm_clip = np.mean(np.asarray(norms))
        # if l2_norm_clip < 0.1:
        #     l2_norm_clip = 0.1
        # if l2_norm_clip > 8:
        #     l2_norm_clip = 8
    ...
    #### dynamic noise scale for fed-sdp
   
    
    noise_multiplier_cur=-10.15*round/100+15   #linear
    #noise_multiplier_cur=-1.015*round//10+15   #staircase with 10 rounds
    #noise_multiplier_cur=np.exp((100-round)*1.13/100+1.58))   #exponential
    #noise_multiplier_cur=15*np.cos(round*1.24/100)   #cyclic
    ...
    #
    #                 ##### real l2 distance
    #                 # logitsaaa = model(tf.expand_dims(x[idx+1], 0))
    #                 # loss_valueaaa = loss_fn(y[idx+1], logits)
    #                 # gradientsaaa = tape.gradient(loss_valueaaa, model.trainable_weights)
    #                 #
    #                 # l2_distance = [np.linalg.norm(gradients[i]-gradientsaaa[i]) for i in range(num_weights)]
    #                 # print ('label',y[idx].numpy(),'and',y[idx+1].numpy())
    #                 # print ('layer-wise l2_distance',l2_distance,'\n')
    #
    #
    #
    ...
    #
    #
    #             # ########if use mean clipping or min clipping or l2 max
    #             # processed_norms = [tf.expand_dims(tf.squeeze(norms[i]),0) for i in range(num_weights)]
    #             # for ccc,item in enumerate(processed_norms):
    #             #    if ccc==0:
    #             #         concat_norms= item
    #             #     #elif ccc%2==0:
    #             #     #    concat_norms=tf.concat([concat_norms,item],axis=0)
    #             #     else:
    #             #         concat_norms = tf.concat([concat_norms, item], axis=0)
    #             # l2_norm_clip = np.max(np.asarray(concat_norms),axis=0)
    #             # l2_norm_clip = np.mean(np.asarray(concat_norms),axis=0)
    #             # #print (l2_norm_clip)
    #             # #l2_norm_clip = np.min(np.asarray(concat_norms),axis=0)
    #             # l2_norm_clip[np.where(l2_norm_clip<0.1)] = 0.1
    #             # l2_norm_clip[np.where(l2_norm_clip>32)] = 32
    #
    #
    #
    #
    ...
    #
    #
    #         #print (np.max(l2_norm_clip,axis=0))
    #         #print(np.mean(l2_norm_clip,axis=0))
    #         ######################################clipping used in noise, quadric mean
    #         #from math import sqrt
    #         #noise_clipping = sqrt(sum(n*n for n in l2_norm_clip)/len(l2_norm_clip))
    #
    #         #### dynamic noise scale for fed-cdp
    #         it = round*100 + local_iter
    #
    #         noise_multiplier_cur=-10.15*it/10000+15   #linear
    #         #noise_multiplier_cur=-1.015*round//10+15   #staircase with 10 rounds
    #         #noise_multiplier_cur=np.exp((10000-it)*1.13/10000+1.58))   #exponential
    #         #noise_multiplier_cur=15*np.cos(it*1.24/10000)   #cyclic
    #
    #         ############## if dynamic clipping
    #         # GaussianNoises = [
    #         #     1.0 / x.shape[0] * np.random.normal(loc=0.0, scale=float(noise_multiplier * np.mean(l2_norm_clip,axis=0)),
